...
import {
  ...
  formatSize,
  formatTime,
} from "@/core/utils/quick_formaters";
...
function TorrentCard({
  ...
  return (
    <li>
      <h3>
        {type}.{quality} - {formatSize(size_bytes)} -{" "}
        {calculateAvailability(seeds, peers)}
      </h3>
      <p>{hash}</p>
...
export default function MoviePage({ params: { imdb_id } }: MoviePageProps) {
  const { movie, error, isLoading } = useMovie(imdb_id);

  if (isLoading) return <p>Loading your movie...</p>;
  ...
  return (
    <div style={style}>
      ...
      <h2>Torrents:</h2>
      <ul>
        {movie.torrents.map((t: YtsTorrent) => (
