...
const moviesReducer = (
  ...
  switch (action.type) {
    ...
    case "next":
      if (state.page >= state.pages || state.state !== "data") return state;
      return {
        ...state,
        state: "loading",
        query: state.query || "",
...
const fetcher = <T>(url: string) =>
  fetch(`https://yts.mx/api/v2/${url}`).then((res) =>
    res.ok ? (res.json() as Promise<YtsResponse<T>>) : undefined
  );
...
export function useMovies() {
  ...
  useEffect(() => {
    ...
    fetcher<YtsSearchResult>(
      ...
      .catch(() =>
        dispatch({ type: "error", error: "Failed to fetch movies" })
      );
  }, [state]);
...
export function useMovie(id: string) {
  const { data, isLoading, error } = useQuery({
    queryKey: [`movie_details.json?imdb_id=${id}`],
    queryFn: () => fetcher<YtsMovieResult>(`movie_details.json?imdb_id=${id}`),
  });
